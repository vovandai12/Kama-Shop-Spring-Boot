...
function deleteCategoryBlog(id, name) {
	swal({
		title: "Are you sure?",
		text: "You have want to delete category blog " + name + ". You will not be able to recover this imaginary file!",
		icon: "warning",
		dangerMode: !0,
		buttons: {
			cancel: "No, Please!",
		...
		e ? $.ajax({
			type: "DELETE",
			url: baseUrl + "/admin/category-blog/delete/" + id,
			cache: false,
			success: function() {
...
function viewApiCategoryBlog(id) {
	let viewApi = baseUrl + "/admin/category-blog/view/";
	$('.modal').modal();
	$.ajax({
		type: "GET",
