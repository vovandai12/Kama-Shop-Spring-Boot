...
function deleteCategoryProduct(id, name) {
	swal({
		title: "Are you sure?",
		text: "You have want to delete category product " + name + ". You will not be able to recover this imaginary file!",
		icon: "warning",
		dangerMode: !0,
		buttons: {
			cancel: "No, Please!",
		...
		e ? $.ajax({
			type: "DELETE",
			url: baseUrl + "/admin/category-product/delete/" + id,
			cache: false,
			success: function() {
...
function viewApiCategoryProduct(id) {
	let viewApi = baseUrl + "/admin/category-product/view/";
	$('.modal').modal();
	$.ajax({
		type: "GET",
