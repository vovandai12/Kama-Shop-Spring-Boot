		...
		<div class="col s12 m12 l12">
			<div id="responsive-table" class="card card card-default scrollspy">
				<div class="card-content">
					<h2 class="card-title">Like list</h2>
					<div class="row">
						<div class="col s12">
							<div class="row">
							...
							<table class="responsive-table highlight">
								<thead>
									<tr>
										<th>#</th>
										<th><a
											href="/admin/like?field=user.fullName&keywords=${keywords }&size=${size}">Full
												name</a></th>
										<th>Avatar</th>
										<th><a
											href="/admin/like?field=product.name&keywords=${keywords }&size=${size}">Product
												name</a></th>
										<th>Banner</th>
										<th><a
											href="/admin/like?field=createdDate&keywords=${keywords }&size=${size}">Like
												day</a></th>
										<th>Action</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach var="item" items="${resultPage.content }"
										...
										<tr>
											...
											<td><img src="/uploads/${item.user.avatar }"
											...
											<td>
												<div class="invoice-action">
													<a data-id="${item.id}" data-name="${item.user.fullName}"
														onclick="deleteLike(this.getAttribute('data-id'), this.getAttribute('data-name'))"
							...
							<c:if test="${resultPage.totalPages > 0}">
								<ul class="pagination center-align">
									<li
										...
										<a
										href="/admin/like?field=${field }&keywords=${keywords }&size=${size}&page=1"><i
									...
									<c:forEach var="pageNumber" items="${pageNumbers }">
										<c:if test="${resultPage.totalPages > 1}">
											<li class="waves-effect active"
												...
												<a
												href="/admin/like?field=${field }&keywords=${keywords }&size=${size}&page=${pageNumber }">${pageNumber}
											</a>
											</li>
										</c:if>
									</c:forEach>
									...
									<li
										...
										<a
										href="/admin/like?field=${field }&keywords=${keywords }&size=${size}&page=${resultPage.totalPages }"><i
